$(function () {
    ...
    $(".editable-input").on("blur", function () {
        ...
        let validateInputNumber = inputFieldValue.val();
        if($.isNumeric(validateInputNumber))
            newQuantity = parseFloat(validateInputNumber);
        else
            newQuantity = 0;

        if (parseFloat(quantity) == parseFloat(newQuantity)) return;
        ...
        $.ajax({
            ...
            success: function (ddata) {
                ...
                if (ddata && ddata != []) {
                    ...
                    $("#previewTotal").html("Total $" +($("#totalPrice").html()));
                }
            },
            beforeSend: function () {
                $.blockUI();
